...
import { collection, query, where, onSnapshot, getDocs, deleteDoc, doc, Timestamp } from "firebase/firestore"
...
export default function LocationProvider({ children }) {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          console.log("City detected successfully:", detectedCity, isAutoRetry ? "(auto-retry)" : "")
    ...
    const setupPermissionMonitoring = async () => {
      ...
      try {
        ...
        const handlePermissionChange = () => {
          console.log("Permission state changed:", permission.state, "from:", lastPermissionState)

          if (lastPermissionState === "denied" && permission.state === "granted") {
            console.log("🚀 Permission granted! Auto-retrying location detection...")
            setShowLocationPrompt(false)
            setTimeout(() => {
              detectLocation(true, true)
            }, 1000)
          } else if (lastPermissionState === "granted" && permission.state === "denied") {
            console.log("❌ Permission denied! Switching to all cities mode...")
  ...
  useEffect(() => {
    const handleVisibilityChange = async () => {
      if (document.visibilityState === "visible" && locationStatus === "denied") {
        console.log("Page became visible, checking if permission was granted...")
        if (navigator.permissions) {
          try {
            const permission = await navigator.permissions.query({
              name: "geolocation",
            })
            if (permission.state === "granted" && lastPermissionState === "denied") {
              console.log("🎉 Permission was granted while away! Auto-retrying...")
              setShowLocationPrompt(false)
              setTimeout(() => {
                detectLocation(true, true)
              }, 500)
            ...
            console.log("Permission check on visibility change failed:", error.message)
          }
        }
      }
    }
  ...
  useEffect(() => {
    ...
    const q = query(collection(firestore, "donations"), where("confirmed", "==", true))
    ...
    const unsubscribe = onSnapshot(
      ...
      (querySnapshot) => {
        ...
        console.log("Donations updated:", Object.keys(donationsMap).length, "requests have donations")
      ...
      try {
        ...
        snapshot.forEach((docSnap) => {
          ...
          if (fulfilledAt && (now.seconds - fulfilledAt.seconds) / 3600 >= 24) {
            cleanupPromises.push(deleteDoc(doc(firestore, "requests", docSnap.id)))
          }
        })

        if (cleanupPromises.length > 0) {
          await Promise.all(cleanupPromises)
          console.log(`Cleaned up ${cleanupPromises.length} old fulfilled requests`)
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const q = query(collection(firestore, "requests"), where("deleted", "!=", true))
      ...
      async (querySnapshot) => {
        try {
          ...
          const shouldShowAllRequests =
            (!hasUserFilter &&
              (locationStatus === "denied" || locationStatus === "error" || locationStatus === "not_supported")) ||
  ...
  }, [effectiveCity, orphanagesData, locationStatus, userCityFilter, city, donationsData])
