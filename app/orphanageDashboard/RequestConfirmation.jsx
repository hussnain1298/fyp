...
export default function RequestConfirmations({ activeStatus }) {
        ...
        await Promise.all(
          uniqueDonorIds.map(async (donorId) => {
            try {
              ...
              donorNameMap[donorId] = userDoc.exists() && userDoc.data().name ? userDoc.data().name : null
            } catch {
  ...
  const renderStatusBadge = (status) => {
    ...
    return (
      <span className={`inline-block px-3 py-1 rounded-full font-semibold text-xs ${bgClass} w-max`}>{status}</span>
    )
  }
  ...
  return (
    <section className="mb-10">
      <h3 className="text-2xl font-semibold mb-6 border-b border-gray-300 pb-2 text-gray-900">Requests</h3>
            ...
            return (
              ...
              >
                <div className="space-y-1 text-gray-800">
                  ...
                  <p>Donor: {donorNames[donation.donorId] || donation.donorId}</p>
                  {donation.amount && <p>Amount: Rs. {donation.amount}</p>}
                  {donation.numClothes && <p>Clothes: {donation.numClothes}</p>}
                  {donation.numMeals != null && <p>No of Meals: {donation.numMeals}</p>}
                  {donation.foodDescription && <p>Food: {donation.foodDescription}</p>}
                  <p>Status: {renderStatusBadge(donation.status || (donation.confirmed ? "approved" : "pending"))}</p>
                </div>
