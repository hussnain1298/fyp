...
export default function LiveDonationFeedSlider() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      setDonations([...enriched, ...enriched, ...enriched]);
    });
  ...
  const donationItems = useMemo(() => {
    return donations.map((d, i) => (
      ...
      >
        ...
        <strong className="text-blue-700">{formatDonationAmount(d)}</strong> to{" "}
        <strong className="text-purple-700">{d.orphanageName}</strong>.
      </span>
    ));
  }, [donations]);
  ...
  return (
    <div className="w-screen overflow-hidden py-4 bg-transparent">
      <div className="relative">
        <div
          className={`inline-flex items-center space-x-8 ${
            isPaused ? "pause-animation" : "animate-scroll"
          }`}
          style={{ minWidth: "100%", paddingLeft: "100%" }}
        >
          {donationItems}
        </div>
      </div>
