...
export default function FulfillRequests() {
  ...
  const handleFulfill = async (request) => {
    ...
    // Validation for Money, Clothes, and Food
    if (
      (request.requestType === "Money" || request.requestType === "Clothes" || request.requestType === "Food") &&
      ...
      return alert(`Enter a valid ${typeText}`)
    }
          ...
          <div className="space-y-6">
            {paginatedRequests.map((r) => (
              <div key={r.id} className="bg-gray-100 p-6 rounded shadow relative">
                ...
                {/* Show donated/total for all request types that have quantity */}
                {r.quantity && (
                  <p className="text-sm mt-2 font-medium text-blue-600">
                    Donated: {r.totalDonated || 0} of {r.quantity}{" "}
                    {r.requestType === "Food" ? "meals" : r.requestType === "Clothes" ? "items" : "amount"}
                  </p>
                )}

                <p className="text-sm text-gray-600">
                  <strong>Orphanage:</strong> {r.orphanInfo?.orgName || "N/A"}
                </p>
                <p className="text-sm text-gray-600 mb-2">
                  <strong>Location:</strong> {r.orphanInfo?.city || "N/A"}
                </p>
                <button
                  ...
                  <div className="fixed inset-0 z-50 bg-black bg-opacity-40 flex items-center justify-center">
                    <div className="bg-white p-6 rounded shadow-xl w-full max-w-md">
                      ...
                      <Textarea
                        ...
                        rows={3}
                      />
                      {/* Show amount input for Money, Clothes, and Food */}
                      {["Money", "Clothes", "Food"].includes(r.requestType) && (
                        <div className="mt-4">
